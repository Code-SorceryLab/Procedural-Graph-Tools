...
func _init() -> void:
	...
	_folders_vbox = VBoxContainer.new()
	_folders_vbox.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	scroll.add_child(_folders_vbox)
	
	clear()

func _clear_folders() -> void:
	for child in _folders_vbox.get_children():
		child.queue_free()
	_current_report_raw = ""

func clear() -> void:
	_clear_folders()
	_add_folder("Status", "[color=gray]No progression data available. Rasterize the graph to view the report.[/color]", true)

func set_loading() -> void:
	_clear_folders()
	_add_folder("Status", "[color=gray]Generating and Validating Map...[/color]", true)

func update_report(data: Dictionary) -> void:
	_clear_folders()
	
	if data.is_empty():
		_add_folder("Status", "[color=gray]Progression solver was disabled or failed to find regions.[/color]", true)
		return
		
	# 1. Metadata
	var meta = data.get("meta", {})
	if not meta.is_empty():
		_add_folder("Generation Metadata", _format_metadata(meta), true)
		
	# 2. Multi-Point Validation Analytics
	var multi_analytics = data.get("multi_point_analytics", [])
	if multi_analytics.is_empty():
		var legacy_analytics = data.get("analytics", {})
		if not legacy_analytics.is_empty():
			_add_folder("Validation Analytics", _format_validation(legacy_analytics), true)
	else:
		for i in range(multi_analytics.size()):
			var analytics = multi_analytics[i]
			var cp_name = analytics.get("checkpoint_name", "Checkpoint")
			var title = "Validation: " + cp_name
			# Keep Spawn expanded, collapse triggers
			_add_folder(title, _format_validation(analytics), (i == 0))
			
	# 3. Progression Metrics
	var prog_text = _format_progression(data)
	if prog_text != "":
		_add_folder("Progression Maps & Metrics", prog_text, false)

# ==============================================================================
# UI COMPONENTS
# ==============================================================================
func _add_folder(title: String, content: String, expanded: bool = false) -> void:
	var vbox = VBoxContainer.new()
	vbox.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	
	var btn = Button.new()
	btn.text = ("▼ " if expanded else "▶ ") + title
	...
	btn.pressed.connect(func():
		...
		btn.text = ("▼ " if margin.visible else "▶ ") + title
	)
	
	_folders_vbox.add_child(vbox)
	
	# Append to the raw export string
	_current_report_raw += "=== " + title + " ===\n"
	_current_report_raw += rtl.get_parsed_text() + "\n\n"


# ==============================================================================
# FORMATTING ENGINE
# ==============================================================================
func _format_metadata(meta: Dictionary) -> String:
	var s = ""
...
func _format_validation(analytics: Dictionary) -> String:
	var s = ""
	...
	var init_inv = analytics.get("initial_inventory", [])
	if not init_inv.is_empty():
		s += "  [color=cyan]Starting Inventory:[/color] %s\n" % ", ".join(init_inv)
	else:
		s += "  [color=cyan]Starting Inventory:[/color] Empty\n"
...
func _format_progression(data: Dictionary) -> String:
	var s = ""
	...
	if not multi_doors.is_empty():
		...
		s += "    [color=gray][i]Single physical doors that connect three or more regions.[/i][/color]\n"
	...
	if locks.is_empty():
		...
		for l in locks:
			...
			if is_vault:
				if v_tag == "Fallback (Non-Terminal)": style_tag = "[color=yellow][Vault (Fallback)][/color]"
				elif v_tag == "Non-Terminal Branch": style_tag = "[color=magenta][Vault (Branch)][/color]"
				else: style_tag = "[color=magenta][Vault][/color]"
