...
static func build_ui(settings_list: Array, container: Control, _tooltip_map: Dictionary = {}) -> Dictionary:
	...
	for setting in settings_list:
		...
		# [NEW] Skip Label for Actions (Buttons usually contain their own text)
		if hint_text != "action":
			var label = Label.new()
		...
		if options_str != "":
			...
			# [NEW] CHECK FOR ACTION HINT
			if hint_text == "action":
				var btn = Button.new()
				btn.text = setting.get("label", "Action")
				# Optional: Make it red if it's a destructive action
...
static func collect_params(active_inputs: Dictionary) -> Dictionary:
	...
	for key in active_inputs:
		...
		# [NOTE] We do NOT collect Button states (actions are events, not data)
	return params
...
static func connect_live_updates(active_inputs: Dictionary, callback: Callable) -> void:
	for key in active_inputs:
		...
		# [NEW] Button Signal
			...
			# We pass 'true' just to satisfy the (key, value) signature
